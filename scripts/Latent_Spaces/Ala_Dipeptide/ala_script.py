...
def main(data_file, save_file_prefix, latent_dim):
    ...
    #latent_sizes = [1, 2, 4] #Long training, so separate different latent (specify input)
    latent_sizes = [int(latent_dim),]
    ...
    for l_dim in latent_sizes:
        ...
        for bval in beta_vals:
            #Specify file name to use throughout
            ...
            #But want to be able to split up training so run not too long on cluster
            #To allow for that, check if already have a file and load and move on to next
            #If printed loss information, know training completed
            #(unless it's bval=0 and hasn't trained flow yet, but that's iffy anyway because
            #sometimes flow can't be trained because not in right interval)
            if os.path.isfile('loss_info_'+this_file_name+'.txt'):
                #To load weights correctly, run data through model first to force to build
                unused_recon = vae_model(raw_data[:2])
                del unused_recon
                unused_flow = vae_model.flow(tf.random.normal((2, l_dim)))
                del unused_flow
                print('\n\nLoading model %s'%this_file_name)
                vae_model.load_weights(this_file_name+'.h5')
                #If already trained this bval, skip and go to next
                #But first make sure to update bval (to what it would be after annealing)
                #Or just clean up if bval is zero since won't reuse model
                if bval == 0.0:
                    del vae_model
                else:
                    vae_model.beta = bval
